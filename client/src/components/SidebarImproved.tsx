...
export default function SidebarImproved({ activeSection, onSectionChange }: SidebarProps) {
  ...
  // Lista completa de todos os módulos (CONSOLIDADA - 12 módulos)
  const allModules = [
    // Principais (Core Business)
    { id: 'dashboard', icon: Gauge, label: 'Dashboard & Notificações', badge: null, section: 'main' },
    ...
    { id: 'production', icon: Factory, label: 'Produção & QR Codes', badge: null, section: 'production' },
    { id: 'inventory', icon: Package, label: 'Estoque Inteligente', badge: null, section: 'production' },

    // Análise & Documentos (Módulos Unificados)
    { id: 'reports-analytics', icon: BarChart3, label: 'Relatórios & Analytics', badge: null, section: 'tools' },
    { id: 'operations-schedule', icon: CheckSquare, label: 'Operações & Calendário', badge: null, section: 'tools' },
    ...
    { id: 'admin-backup', icon: Shield, label: 'Administração & Backup', badge: null, section: 'management' },
  ];
    ...
    return (
      <div key={item.id} className="group relative">
        <button
          ...
          className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${activeSection === item.id
